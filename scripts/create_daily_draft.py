...
def get_suggested_post_time(target: date) -> str:
    """Return a stable, date-specific random time between 21:05 and 22:55 JST."""
    start_minutes = 21 * 60 + 5
    end_minutes = 22 * 60 + 55
    random_minutes = random.Random(f"ASOBU-post-time-{target.isoformat()}").randint(start_minutes, end_minutes)
...
def main() -> None:
    ...
    suggested_post_time = get_suggested_post_time(target)
...
🕒 活動：{brand['activity_days']} {brand['activity_time']}／{brand['activity_frequency']}
💰 参加費：{brand['participation_fee']}
🏸 道具：{brand['rental']}
🎒 持ち物：{brand['what_to_bring']}
...
"""
    ...
    posting_time = f"""# 投稿候補時刻｜{target.isoformat()}

**今日の手動投稿候補時刻：** **{suggested_post_time}（日本時間）**
...
- [ ] `posting-time.md`に記載された手動投稿候補時刻（{suggested_post_time}・日本時間）を確認した。
...
**今日のテーマ：** {title}  
**今日の投稿候補時刻：** **{suggested_post_time}（日本時間）**
"""
    ...
    write_text(output_dir / "posting-time.md", posting_time, args.force)
