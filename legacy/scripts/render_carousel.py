...
def draw_point(draw: ImageDraw.ImageDraw, slide: dict[str, Any], brand: dict[str, Any], index: int) -> None:
    draw_header(draw, brand, index, "POINT")
    ...
    draw.text((SIZE / 2, 742), str(slide.get("fact_label", "POINT")), font=font(25, True), fill=COLORS["lime"], anchor="mm")
    fit_text(draw, (150, 760, 930, 842), wrap_japanese(str(slide.get("fact_value", "")), 22), 39, COLORS["white"], spacing=6, align="center")
...
def draw_info(draw: ImageDraw.ImageDraw, slide: dict[str, Any], brand: dict[str, Any], index: int) -> None:
    ...
    fit_text(draw, (145, 868, 935, 937), wrap_japanese(str(slide.get("note", "")), 23), 25, COLORS["dark"], bold=False, spacing=4, align="center")
